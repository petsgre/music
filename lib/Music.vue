<template>
  <div class="my-music">
    <div class="music-box" :key="2">
      ...
      <div class="music-voice" @click="voiceShow=!voiceShow,listShow=false">
      ...
      <div class="music-list" @click.self="listShow=!listShow,voiceShow=false">
        <div class="music-list-result" v-if="listShow">
          <div class="music-search">
            ...
            <div v-if="searchShow" style="float:right;" @click="searchShow=false">取消</div>
            <div v-if="!searchShow" style="float:right;" @click="listShow=false">取消</div>
          </div>
          <li v-if="searchShow" @click="playOnline(searchResult)" style="cursor:pointer;" v-for="(searchResult,index) in searchResults">
            ...
            <div style="width:40px;" v-html="Math.floor(searchResult.Duration/60)+':'+(searchResult.Duration%60/100).toFixed(2).slice(-2)"></div>
          ...
          <li v-if="!searchShow" @click="playOnline(onlineList)" style="cursor:pointer;" v-for="(onlineList,index) in onlineLists">
            ...
            <div style="width:40px;" v-html="Math.floor(onlineList.Duration/60)+':'+(onlineList.Duration%60/100).toFixed(2).slice(-2)"></div>
...
export default {
  data() {
    return {
      searchShow: false,
      audioToggle: true,
      musicUrl: "http://fs.web.kugou.com/3d0bfee19ba670735d59ba592740c025/598d4ab0/G052/M04/19/07/FJQEAFZWxMSAbdi-ADPyIZwJRM0272.mp3",
      musicName: "",
  ...
  methods: {
    ...
    musicSearch() {
      ...
      //百度音乐
      // var search = this.$http.jsonp("http://tingapi.ting.baidu.com/v1/restserver/ting?method=baidu.ting.search.catalogSug&query=" + musicName)
      //酷狗音乐
      var search = this.$http.jsonp("http://songsearch.kugou.com/song_search_v2?_callback=jQuery191034642999175022426_1489023388639&keyword=" + musicName + "&page=1&pagesize=10&userid=-1&clientver=&platform=WebFilter&tag=em&filter=2&iscorrection=1&privilege_filter=0&_=1489023388641")
      search.then(data => {
        if (data.body.error_code == 0) {
          this.searchResults = data.body.data.lists
        }
      })
    },
    playOnline(e) { //播放音乐
      var search = this.$http.get("song/yy/index.php?r=play/getdata&hash=" + e.FileHash)
      search.then(data => {
        if (data.body.err_code == 0) {
          this.musicUrl = data.body.data.play_url;
          var music = new Music();
          music.reload();
          this.duration = e.Duration;
          this.musicHash = e.FileHash;
          this.audioToggle = true;
          if(data.body.data.play_url == ""){
            alert("很遗憾！播放源地址无效！");
          }
        }
      }).catch(data => {
  ...
  mounted: function() {
    ...
    if (list.length !== 0) {
      ...
      this.duration = 213; //默认长度是fade的长度
    }
    //监听音乐正在播放
    var audio = document.getElementById("audio");
    var vm = this;
